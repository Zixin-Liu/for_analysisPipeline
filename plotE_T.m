function plotE_T(allSubBehavData, samples)
% This is a simple function that takes the actual Estimation Error and
% simulated Estimation Error and calculate spearman's correlation with
% according graph generated.


realET = allSubBehavData.e_t;     
predET = samples.e_t;    
% Ensure the et does not contain NAN

valid_idx = ~isnan(realET) & ~isnan(predET);
realET = realET(valid_idx);
predET = predET(valid_idx);
